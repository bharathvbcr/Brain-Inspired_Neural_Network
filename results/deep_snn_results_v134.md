...
> **SUPERSEDED BY v135 — 2026-08-21.** The measurements below are unchanged and
> correct; two of the *interpretations* were not.
>
> - The depth-4 ceiling row read **`ok`**. It is a constant predictor
>   (`0.5000 ± 0.0000` on a two-class task). The health check was
>   `ceiling < treatment`, which is silent when the reference is dead and the
>   treatment is below it.
> - The 1-hidden-layer arm read **PASS** against a reference at 0.4880. The
>   verdict took `harness_valid = true` unconditionally.
>
> Both are fixed in [`deep_snn_results_v135.md`](deep_snn_results_v135.md), which
> reports the same accuracies with every ceiling flagged and every arm
> `INVALID_HARNESS`. See
> [`HARDENING_2026-08-21_CEILING_HEALTH_HAS_ONE_OWNER.md`](HARDENING_2026-08-21_CEILING_HEALTH_HAS_ONE_OWNER.md).
