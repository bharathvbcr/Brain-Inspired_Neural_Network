...
>
> The e-prop ceiling used `wout ~ U[-1,1]*0.2/sqrt(h)` as its hidden modulator while the DFA arm used `B ~ U[-1,1]` -- a ~56x effective-learning-rate deficit at the shared lr = 0.02. That is why the ceiling scored below the treatment. Any DFA-vs-ceiling reading here is invalid. The Reproduce block also omits `--shd-hidden`.
>
